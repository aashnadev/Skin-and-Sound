...
int DSensor = A2;    // pin that the sensor is attached to
int led = 10;       // pin that the LED is attached to
int PSensor = A0;   // an arbitrary threshold level that's in the range of the analog input
int analogValueD;
int analogValueP;
...
int mod_ratio = 3 ; // brightness (harmonics)
...
void loop(){
  audioHook();
  analogValueD = mozziAnalogRead(DSensor);
analogValueP = mozziAnalogRead(PSensor);
